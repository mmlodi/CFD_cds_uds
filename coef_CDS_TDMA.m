function [a_n, a_s, a_e, a_w, a_p, s_u, s_p, max_peclet_u_CDS, max_peclet_v_CDS] = coef_CDS_TDMA( dx, dy, div_x, n_vol, c_p, k_ele, rho, T_N_vec, T_S_vec, T_E_vec, T_W_vec, angle, r_mat, omega)
    ...
    peclet_u = (V_u.*rho)./(k_ele/(c_p*dx));
    peclet_v = (V_v.*rho)./(k_ele/(c_p*dy));
    
    max_peclet_u_CDS = max(max(abs(peclet_u)));
    max_peclet_v_CDS = max(max(abs(peclet_v)));
    ...
    for i = 1:n_vol % MONTA O CADA VETOR A_N, A_S, A_W, A_E, A_P, S_P e S_U PARA CADA ELEMENTO
        ...
        if mod(i,div_x) == 0 % MONTANDO VETORES PARA PAREDE DIREITA
            ...
            s_u(div_x) =  - Fy(div_x)*T_N_vec(div_x) + (2*Dy)*T_N_vec(div_x) - Fx(div_x)*T_E_vec(div_x) + (2*Dx)*T_E_vec(div_x);% CORREÇÂO PARA O ULTIMO ELEMENTO
            s_u(n_vol) =  + Fy(n_vol)*T_S_vec(n_vol) + (2*Dy)*T_S_vec(n_vol) - Fx(n_vol)*T_E_vec(n_vol) + (2*Dx)*T_E_vec(n_vol);

            s_p(div_x) = -(  - Fy(div_x) - Fx(div_x) + (2*Dy) + (2*Dx));% CORREÇÂO PARA O ULTIMO ELEMENTO
            s_p(n_vol) = -(  + Fy(n_vol) - Fx(n_vol) + (2*Dy) + (2*Dx));
        ...
        elseif mod(i,div_x) == 1 % MONTANDO VETORES PARA PAREDE ESQUERDA
            ...
            s_p(1) = - ( - Fy(1) + Fx(1) + (2*Dy) + (2*Dx)) ;% CORREÇÂO PARA O PRIMEIRO ELEMENT
            s_p(n_vol - div_x+1) = - ( + Fy(n_vol-div_x+1) + Fx(n_vol-div_x+1) + (2*Dx) + (2*Dy));% CORREÇÂO PARA O PRIMEIRO ELEMENTO DA ULTIMA LINHA
        ...
        elseif (i > 1) && (i < div_x)%SUPERIOR SEM AS LATERAIS
            ...
            a_w(i) = + Fx(i)/2 + Dx; % F = rho V_u ou V_v
            a_e(i) = - Fx(i)/2 + Dx; 
            a_n(i) = 0;
            a_s(i) = + Fy(i)/2 + Dy;
        ...
        elseif (i > ( n_vol - div_x + 1 )) && (i < n_vol )%INFERIOR SEM AS LATERAIS
            ...
            s_u(i) = + Fy(i)*T_S_vec(i) + (2*Dy)*T_S_vec(i);
            s_p(i) = -(  + Fy(i) +   2*Dy);
